...
const http = require('https').createServer({
  key: fs.readFileSync('/etc/letsencrypt/live/colorballnumbers.com/privkey.pem'),
  cert: fs.readFileSync('/etc/letsencrypt/live/colorballnumbers.com/cert.pem'),
  ca: fs.readFileSync('/etc/letsencrypt/live/colorballnumbers.com/chain.pem')
}, app);
...
const standardCredit = 5;  // 1 for testing; 5 for live runs   Changed to 5 per Alan, 2024-11-17
var credit_result = 0;
...
//        console.log('credit: ' + results[0]['credit']);
        ...
        response.cookie('access', `${results[0]['access']}`);

        var selected_id = '';

        //
        // Start off this login session with 4 credits...
        //
...
//        });
        ...
        mysqlConn.query(`select * from player_logins where player_id = ${req.session.uvi} and logged_out = 0 order by date_time desc`, (err2, res2) => {
          if (res2 != null && res2 != '' & typeof res2 != "undefined") {
            selected_id = res2[0]['player_id'];
            ...
            mysqlConn.query(`insert into player_logins (player_id, logged_out, date_time, daily_plays) values (${req.session.uvi}, 0, ${mysqlConn.escape(nowDate)}, ${standardCredit})`, (err3, res3, fields) => {
              if (res3) {
                //console.log('Inserted id ' + res3['insertId'] + ' into `player_logins`');
                //resolve(res['insertId']);
                //
                // Start off this login session with 4 credits
                //
                //mysqlConn.query(`update player_credit set credit = (credit + ${standardCredit}) where player_id = ${req.session.uvi}`, (err4, res4) => {
                mysqlConn.query(`update player_credit set credit = ${standardCredit} where player_id = ${req.session.uvi}`, (err4, res4) => {
                  //console.log('err4: ' + err4);
                });
                //response.redirect('/play');
                response.redirect('/videoAds');
              }
              if (err3) {
...
app.get('/playerGet', (req, res) => {
  let uvi = req.session.uvi;
  if (uvi == 0 || uvi == null || typeof uvi == "undefined" || uvi == '') {
    return res.sendFile(__dirname + '/home.html');
  }
  return res.sendFile(__dirname + '/players.html');
});

app.get('/playerList', (req, res) => {
//  console.log('At top of playerList...');
//  const dom = new JSDOM(`<!DOCTYPE html><p>Hello world</p>`);
//  console.log(dom.window.document.querySelector("p").textContent);
//  const { window } = new JSDOM(`<!DOCTYPE html><p>Hello world</p>`);
//  const { document } = (new JSDOM(`<!DOCTYPE html><p>Hello world</p>`)).window;
//  const { navigator } = (new JSDOM(`<!DOCTYPE html><p>Hello world</p>`)).window;
//  console.log(navigator.appName);
  let myResults = 'temp';
  mysqlConn.query(`select * from players`, async (err, results, fields) => {
    //console.log('err is ' + err);
    if (err) {
    ...
    myResults = results;
    if (results.length > 0) {
      console.log('Stand by for players list...');
      var playersTable = '<table id="playersTable">';
      playersTable += '<thead>';
      playersTable += '<tr>';
      playersTable += '<th>Id</th>';
      playersTable += '<th>Name</th>';
      playersTable += '<th>Email</th>';
      playersTable += '<th>Postal Code</th>';
      playersTable += '<th>Sponsor Email</th>';
      playersTable += '</tr>';
      playersTable += '</thead>';
      playersTable += '<tbody>';
      for (var i = 0; i < results.length; i++) {
          playersTable += "<tr><td>" + results[i]['id'] + "</td><td>" +  results[i]['name'] + "</td><td>" + results[i]['email'] + "</td><td>" + results[i]['postalCode'] + "</td><td>" + results[i]['sponsor_email'] + '</td></tr>';
      }
      playersTable += '</tbody>';
      playersTable += "</table>";
//      console.log(playersTable);
      return res.json(playersTable);
    } else {
...
//     console.log('### credit_result after credit query: ' + credit_result);
    ...
    if (credit_result >= 0) {  // Use false && to bypass this code    11-08-2023
      ...
      const last_24_hours = await new Promise((resolve, reject) => {
        ...
        let tzOffset = -oneDayAgoOnMidnight.getTimezoneOffset() / 60;
...
//        oneDayAgoOnMidnight.setSeconds(0);
        ...
        let rawHours = oneDayAgoOnMidnight.getHours();
//        let adjustForPartialDay = rawHours - 2 + tzOffset;  // 2 is diff betw 24 and 26
        let adjustForPartialDay = rawHours + 2 + tzOffset;  // 2 is diff betw 24 and 22
        if (adjustForPartialDay < 0) {
          adjustForPartialDay += 24;
        }

        let queryLimitDate = '' + oneDayAgoOnMidnight.getFullYear() + '-' + (oneDayAgoOnMidnight.getMonth() + 1) + '-' + `${padWithTwoZeros(oneDayAgoOnMidnight.getDate())}` + ' ' + `${padWithTwoZeros(adjustForPartialDay)}` + ':' +
        `${padWithTwoZeros(oneDayAgoOnMidnight.getMinutes())}` + ':' +
        `${padWithTwoZeros(oneDayAgoOnMidnight.getSeconds())}`;
      ...
      var gameNumber = `${padWithZeros(req.body.gameNumber)}`;
      var thisWinningNumber = req.body.winningNumber;
      var player_id = `${req.session.uvi}`;
      var selection = `${padWithZeros(req.body.numText)}`;
      var winning_number = `${padWithZeros(thisWinningNumber)}`;
      //console.log('### gameNumber: ' + gameNumber);
      //console.log('### thisWinningNumber: ' + thisWinningNumber);
      if (typeof thisWinningNumber != "undefined" && typeof gameNumber != "undefined") {
        console.log('### Inserting into `player_selections`: player_id: ' + player_id + ' selection: ' + selection + ' winning_number: ' + winning_number + ' gameNumber: ' + gameNumber);
        mysqlConn.query(`insert into player_selections (player_id, selection, winning_number, drawing_id, drawing_num, date_time) values (${player_id}, ${selection}, ${winning_number}, 0, ${gameNumber}, ${mysqlConn.escape(new Date(Date.now()))})`);
      }
...
app.get('/credit', async (req, res) => {
  ...
  if (authHeader) {
    ...
    if (req.session.user == 'demo') {
       ...
       credit = await new Promise((resolve, reject) => {
...
app.post('/gender', async (req, res) => {
  ...
  if (authHeader) {
    ...
    console.log('name is ' + validToken.name + ' and user is ' + req.session.user);
    if (validToken.name !== req.session.user) {
    	console.log('user mismatch in /gender');
      return res.json({ gender: 'N/A', user: 'bar' });
    }
    if (req.session.user == 'demo') {
       console.log('user is demo and gender is N/A in /gender');
       return res.json({ gender: 'N/A', user: 'demo' });
    } else {
       pronoun = await new Promise((resolve, reject) => {
         //mysqlConn.query(`select daily_plays from player_logins where player_id = ${req.session.uvi} order by id desc limit 1`, (err, results, fields) => {
         console.log('user id is ' + `${req.session.uvi}`);
         mysqlConn.query(`select pronoun from players where id = ${req.session.uvi}`, (err, results, fields) => {
            if (err) {
              console.error(err);
              return false;
            }
            console.log('results in /gender is ' + results);
            for (let x in results) {
            	console.log('x: ' + x + ' -- ' + results[x]);
            	for (let y in results[x]) {
            		console.log('y: ' + y + ' -- ' + results[x][y]);
            	}
            }
            if (results != null && results != '' && typeof results != 'undefined') {
              resolve(results[0]['pronoun']);
            }
          });
        });
        gender = pronoun;
        console.log('Before return in /gender: gender is ' + gender);
        return res.json({ gender: `${pronoun}`, user: `${req.session.user}`});
      }
    } else {
    	console.log('authHeader problem in /gender...');
    }
});
...
app.post('/tallyZero4', async (req, res) => {
  const authHeader = req.cookies['accessToken'];
  const token = authHeader && authHeader.split(' ')[1];
  //console.log('### authHeader: ' + authHeader);
  if (authHeader) {
    ...
    mysqlConn.query(`insert into zero_tally (player_id, number_of_digits, date_time) values (${req.session.uvi}, 4, ${mysqlConn.escape(new Date(Date.now()))})`);
  }
});

app.post('/tallyZero3', async (req, res) => {
  const authHeader = req.cookies['accessToken'];
  const token = authHeader && authHeader.split(' ')[1];
  //console.log('### authHeader: ' + authHeader);
  if (authHeader) {
    ...
    mysqlConn.query(`insert into zero_tally (player_id, number_of_digits, date_time) values (${req.session.uvi}, 3, ${mysqlConn.escape(new Date(Date.now()))})`);
  }
});
...
app.get('/get_tally', async (req, res) => {
  ...
  if (authHeader) {
    ...
    //  mysqlConn.query(`select count(*) as tallyCount4 from zero_tally WHERE number_of_digits = 4, count(*) as tallyCount3 from zero_tally WHERE number_of_digits = 3`, (err, res) => {
    tallyCount4 = await new Promise((resolve, reject) => {
      mysqlConn.query(`select count(*) as tallyCount4 from zero_tally WHERE number_of_digits = 4`, (err, res) => {
        if (res != null && res != '' & typeof res != "undefined") {
          resolve(res[0]['tallyCount4']);
        }
        if (err) return false;
      });
    });

    tallyCount3 = await new Promise((resolve, reject) => {
      mysqlConn.query(`select count(*) as tallyCount3 from zero_tally WHERE number_of_digits = 3`, (err, res) => {
        if (res != null && res != '' & typeof res != "undefined") {
          resolve(res[0]['tallyCount3']);
        }
        if (err) return false;
      });
    });

    return res.json({ tallyCount4: tallyCount4, tallyCount3: tallyCount3 });
  }
});
...
app.post('/validateSignup', async (req, res) => {
  ...
  await new Promise((resolve, reject) => {
    mysqlConn.query(`insert into players (name, email, password, access, tempHash, state, postalCode, pronoun, sponsor_email) values (${mysqlConn.escape(req.body.name)}, ${mysqlConn.escape(req.body.email)}, ${mysqlConn.escape(req.body.pass)}, 0, '${tempHash}', ${mysqlConn.escape(req.body.state)}, ${mysqlConn.escape(req.body.postalCode)}, ${mysqlConn.escape(req.body.pronoun)}, ${mysqlConn.escape(req.body.sponsor_email)})`, async (err, results, fields) => {
      if (err) {
      ...
      //sendCustomEmail({ to: `${req.body.email}`, subject: "Email Verification", text: "Please confirm your email", html: `<b>Hello ${req.body.name},</b><br /><a href="https://colorballnumbers.com/emailVerification/${results.insertId}/${tempHash}" target="_blank">Please click this link to verify your email address with Colorball.</a><br /><br />If you are having problems with the link, copy and paste this URL into a browser:<br />https://colorballnumbers.com/emailVerification/${results.insertId}/${tempHash}` });
...
app.get('/ppt', (req, res) => {
  return res.sendFile(__dirname + '/ppt.html');
});
...
app.get('/videoPage', (req, res) => {
  return res.sendFile(__dirname + '/videoPage.html');
});

//app.get('/screen1', (req, res) => {
//  return res.sendFile(__dirname + '/screen1.html');
//});

app.get('/screen4', (req, res) => {
  return res.sendFile(__dirname + '/screen4.html');
});

app.get('/screenOrder', (req, res) => {
  return res.sendFile(__dirname + '/ScreenOrder.html');
});

app.get('/screen5', (req, res) => {
  try {
    //var mixFile = fs.readFileSync('C:\\Users\\jimli_\\Downloads\\441 (720p).mp4');
    //var mixFile = fs.readFileSync('/storage/emulated/0/Movies/441 (720p).mp4');
    //var mixFile = fs.readFileSync('/Movies/441 (720p).mp4');
    //console.log('mixFile was read.');
    return res.sendFile(__dirname + '/screen5.html');
  } catch (err) {
    console.log('Error reading file:', err);
    return res.sendFile(__dirname + '/readFileError.html');
  }
});
...
app.post('/passReset', (req, res) => {
  ...
  mysqlConn.query(`select * from players where email='${req.body.user}'`, (err1, results1, fields1) => {
    ...
    mysqlConn.query(`update players set tempHash='${tempHash}' where email='${req.body.user}'`, async (err, results, fields) => {
      ...
      try {
        let html = `<b>Hello ${results1[0]['name']},</b><br /><a href="https://colorballnumbers.com/passwordReset/${results1[0]['id']}/${tempHash}" target="_blank">Please click this link to reset your password.</a><br /><br />If you are having problems with the link, copy and paste this URL into a browser:<br />https://colorballnumbers.com/passwordReset/${results1[0]['id']}/${tempHash}`;
...
app.get('/videoAds', (req, res) => {
  return res.sendFile(__dirname + '/videoAds.html');
});

app.get('/Kiosks', (req, res) => {
  return res.sendFile(__dirname + '/Kiosks.html');
});

app.get('/screens', (req, res) => {
  return res.sendFile(__dirname + '/screens.html');
});
    ...
    (async () => {
      while (true) {
        ...
        await ex.wait(delayTime).then(() => {
          ...
          if (typeof lastGameData.id !== "undefined") {
            //console.log('checking for winner');
            //console.log((lastGameData));
            mysqlConn.query(`select p.name, p.email, s.player_id, s.selection, s.winning_number, s.drawing_id, s.drawing_num, s.date_time from player_selections as s join players as p on s.player_id = p.id where s.selection = s.winning_number and s.selection = ${lastGameData.winningNumber}`, (err, results, fields) => {
              if (err) {
