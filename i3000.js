...
console.log(`After mysqlConn.connect in i3000...`);
...
function getDrawingNumber(gameTime) {
  ...
  console.log('^^^ start in i3000 getDrawingNumber: ' + start);
...
function getDrawingNumberFromTime(t) {
  ...
  console.log(`now in getDrawingNumberFromTime i3000: `, now);
...
//        let adjustForPartialDay = rawHours - 2 + tzOffset;  // 2 is diff betw 24 and 26
        let adjustForPartialDay = rawHours + 2 + tzOffset;  // 2 is diff betw 24 and 22
                                       // We're using 22 hours as a limit
...
app.get('/showAd', (req, res) => {
  var menVideos = dirInfo.children;
  var videoList = JSON.stringify(menVideos);
  sessionstorage.setItem("videoList", videoList);
  console.log('videoList is ' + videoList);
  return res.sendFile(__dirname + '/showAd.html');
});
  ...
  if (!started) {
    ...
    (async () => {
      while (true) {
        ...
        delayTime *= 1000;
        console.log(`initial delay time: ${delayTime}`);
        // Fake it locally...
        //delayTime = 1000;
        ...
        await ex.wait(delayTime).then(() => {
          ...
          //sendAll({ title: 'openBets', data: { close: false } });
          ...
          await ex.wait(1000).then(() => {
            console.log('in i3000.js... sending startVids...');
