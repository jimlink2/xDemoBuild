...
document.getElementById("ReplayButton").addEventListener('click', replayGame, false);
document.getElementById("TooManyReplays").addEventListener('click', backToStart, false);
...
function replayGame() {
  ...
  window.location.href = "Screen3.html?faveNums=" + faveNums + "&pronoun=" + pronoun + '&lastFour=' + lastFour;
}
...
// Centralized helper for creating a Vimeo player with retry logic
function createSafePlayer(domElement, options, getFreshVideoId) {
  const player = new Player(domElement, options);

  // Track play/pause state
  player.on('play', () => console.log(`${domElement.id} is playing...`));
  player.on('pause', () => console.log(`${domElement.id} is paused...`));

  // Handle errors
  player.on('error', async (err) => {
    console.log(`${domElement.id} error:`, err);

    if (err && err.name === 'UnauthorizedError') {
      try {
        // Ask backend for a fresh video ID or signed playback token
        const newVideoId = await getFreshVideoId(domElement.id);

        // Retry with new ID
        await player.loadVideo(newVideoId);
        console.log(`${domElement.id} recovered with new video ID`);
      } catch (retryErr) {
        console.log(`${domElement.id} retry failed:`, retryErr);
      }
    }
  });
...
function stopMovies() {
    //window.location.href = "Screen1.html?tenDigit=" + tenDigit + "&faveNums=" + faveNums + "&pronoun=" + pronoun;
    window.location.href = "Screen1.html?faveNums=" + faveNums + "&pronoun=" + pronoun + '&lastFour=' + lastFour;
}
...
function nextMovie(movieNumber) {
    ...
    if (movieNumber === false) {
      ...
      document.getElementById('winning_number').innerHTML = `Winning No. ${circleStr}`;
      
      var cookieNameLastFour = 'phone_plays_' + lastFour;
      var phone_plays = getCookie(cookieNameLastFour);
      console.log('In index2, ' + cookieNameLastFour + ' is ' + phone_plays);

      if (faveNums == combinedNums) {
    ...
    switch (movieNumber) {
        case 1:
          if (video1handled == false) {
            video1handled = true;
            nextMovie(2);  // For this version, we are
                           // skipping video 1
            break;
            ...
            player1.on('error', async (err) => {
              console.log("Player1 error:", err);
              if (err && err.name === 'UnauthorizedError') {
                const newVideoId = tempInitialRandomVidIds[0];
                player1.loadVideo(newVideoId)
                  .then(() => console.log("Retried with new video ID"))
        ...
        case 2:
          if (video2handled == false) {
            ...
            //tempDoc2 = document.getElementById("vid1_num_b");
            //tempDoc2.style.opacity = 1;
            //tempDoc2.style.opacity = 0;
            //tempDoc2.style.width = "0";
//            lastDigit = getNumberFromId(tempList[0]);
//            console.log('lastDigit: ' + lastDigit);
//            currWin = document.getElementById('winning_number').innerHTML;
//            if (currWin.substring(0,20) == 'Winning No. <span cl') {
...
//            }
            ...
            player2.on('error', async (err) => {
              console.log("Player2 error:", err);
              if (err && err.name === 'UnauthorizedError') {
                const newVideoId = tempInitialRandomVidIds[1];
                player2.loadVideo(newVideoId)
                  .then(() => console.log("Retried with new video ID"))
                  .catch(e => console.log("Retry failed:", e));
              }
            });
        ...
        case 3:
          if (video3handled == false) {
            ...
            tempDoc2.style.opacity = 1;
            lastDigit = getNumberFromId(tempList[1]);
            console.log('lastDigit at 3: ' + lastDigit);
            //currWin = document.getElementById('winning_number').innerHTML;
            //if (currWin.substring(0,20) == 'Winning No. <span cl') {
            currWinDigits = lastDigit;
            console.log("CASE 3: currWinDigits is " + currWinDigits);
            winningNumStr = buildDisplayDigits(''+lastDigit);
            console.log("CASE 3: winningNumStr is " + winningNumStr);
            document.getElementById('winning_number').innerHTML = `Winning No. ${winningNumStr}`;
            //}
//            currWin = document.getElementById('winning_number').innerHTML;
//            document.getElementById('winning_number').innerHTML = currWin + lastDigit;
            //tempDoc = document.getElementById("vid3_num");
//            playerOptions.id = tempInitialRandomVidIds[2];
//            console.log('Video number is ' + tempInitialRandomNums[2]);
//            console.log('Video id is ' + tempInitialRandomVidIds[2]);
            playerOptions.id = tempInitialRandomVidIds[2];
            //playerOptions.id = tempInitialRandomVidIds[1];
            //console.log('Video number is ' + tempInitialRandomNums[1]);
            //console.log('Video id is ' + tempInitialRandomVidIds[1]);
            playerOptions.muted = muted;
            player3 = new Player(movie3, playerOptions);
            player3.on('play', () => { 
              player3Playing = true;
            ...
            player3.on('error', async (err) => {
              console.log("Player3 error:", err);
              if (err && err.name === 'UnauthorizedError') {
                const newVideoId = tempInitialRandomVidIds[2];
                player3.loadVideo(newVideoId)
                  .then(() => console.log("Retried with new video ID"))
                  .catch(e => console.log("Retry failed:", e));
              }
            });
        ...
        case 4:
          if (video4handled == false) {
            ...
            tempDoc2.style.opacity = 1;
            lastDigit = getNumberFromId(tempList[2]);
            console.log('lastDigit at 4 ' + lastDigit);
            console.log("CASE 4a:  currWinDigits is " + currWinDigits);
            currWinDigits = currWinDigits + '' + lastDigit;
            console.log("CASE 4b:  currWinDigits is " + currWinDigits);
            //currWin = document.getElementById('winning_number').innerHTML;
            ...
            //document.getElementById('winning_number').innerHTML = currWin + lastDigit;
//            playerOptions.id = tempInitialRandomVidIds[3];
//            console.log('Video number is ' + tempInitialRandomNums[3]);
//            console.log('Video id is ' + tempInitialRandomVidIds[3]);
            playerOptions.id = tempInitialRandomVidIds[3];
            playerOptions.muted = muted;
            player4 = new Player(movie4, playerOptions);
            player4.on('play', () => { 
            ...
            player4.on('error', async (err) => {
              console.log("Player4 error:", err);
              if (err && err.name === 'UnauthorizedError') {
                const newVideoId = tempInitialRandomVidIds[3];
                player4.loadVideo(newVideoId)
                  .then(() => console.log("Retried with new video ID"))
                  .catch(e => console.log("Retry failed:", e));
              }
            });
...
function buildCircles(faveNumStr) {
  ...
  var circle3 = faveNumStr.substring(2);
  console.log("###1: " + circle1 + " 2: " + circle2 + " 3: " + circle3);
...
function startScript(faveNums, pronoun) {
    ...
    if (typeof faveNums == "undefined" || faveNums == '' || faveNums == null) {
      console.log("@@@ Reconstructing faveNums...");
      var searchStr = window.location.search;
      var ampPos = searchStr.indexOf("&");
      var secondAmpPos = searchStr.lastIndexOf('&');
      var faveNums = searchStr.substring(10, ampPos);
      var pronoun = searchStr.substring(ampPos+9, secondAmpPos);
      var lastFour = searchStr.substring(secondAmpPos+10, secondAmpPos+14);
      var faveNumStr = buildCircles(faveNums);
    } else {
      var faveNumStr = buildCircles(faveNums);
    }
    document.getElementById('faveNums').innerHTML = "Player\'s No. " + faveNumStr;
