...
const clock_info = document.getElementById("clock_text");
const player_info = document.getElementById("player_text");
...
function update_player_and_game(activeReduction) {
    ...
    console.log('^^^ playerGameSubmission is ' + playerGameSubmission);
    if (playerGameSubmission.toString().length < 4) {
      ...
      if (currentGameNumber != '' && currentWinningNumber != '') {
        ...
        httpRequest.open('POST', 'credit_use');
        httpRequest.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        httpRequest.send(JSON.stringify({ amount: 1, numText: playerGameSubmission, winningNumber: currentWinningNumber, gameNumber: currentGameNumber }));

        if (playerGameSubmission == currentWinningNumber) {
          document.getElementById('WinnerAnnouncement').style.display = 'inline-block';
        }
        //document.getElementById('Limit24Reached').style.display = 'inline-block';
      }

      if (activeReduction == true) {   // Use false && to prevent reduction in credits  10-31-2023
        credit_amount--;
        console.log('credit_amount in activeRedution in update_player_and_game() is ' + credit_amount);
        if (credit_amount < 0) {
          credit_amount = 0;
          playerGameSubmission = '----';
    ...
    player_info.innerHTML = `${user_name}&apos;s No. ${playerGameSubmission}&nbsp;&nbsp;Game No. ${displayGameNumber}`;
...
function getDrawingNumberFromTime(t) {
    //console.log('t is ' + t);
    let EasternOffset = -4;  // ET UTC offset during DST
    //let now = new Date(t);
    let now = new Date();
  ...
  if (httpRequest.readyState === XMLHttpRequest.DONE) {
      if (httpRequest.status === 200) {
          ...
          if (typeof data != "undefined") {
            //if (data.credit == -57 || data.msg == 'too many bets') {
            if (credit_amount <= 0) {
              document.getElementById('Limit24Reached').style.display = 'inline-block';
              document.getElementById('credit').innerHTML = `$${data.credit}`;
              credit_amount = parseInt(data.credit, 10);
              console.log('credit_amount A: ' + credit_amount + ' in receiveCredit2()');
            } else {
              document.getElementById('credit').innerHTML = `$${data.credit}`;
              credit_amount = parseInt(data.credit, 10);
              console.log('credit_amount B: ' + credit_amount + ' in receiveCredit2()');
